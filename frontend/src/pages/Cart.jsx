...
const Cart = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
      <h1 className="text-3xl md:text-4xl font-playfair font-bold text-black uppercase tracking-widest mb-12">Shopping Bag</h1>
      ...
      {items.length === 0 ? (
        <div className="text-center py-20 border-t border-b border-gray-100">
          ...
          <Link to="/products">
            <Button size="lg" className="uppercase tracking-widest">Continue Shopping</Button>
          </Link>
        </div>
      ) : (
        <div className="flex flex-col lg:flex-row gap-16">
                  ...
                  <div className="ml-6 flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start">
                        <Link to={`/products/${item.product?._id}`} className="text-lg font-medium text-black hover:underline underline-offset-4 decoration-1">
                          {item.product?.name}
                        </Link>
                        <p className="text-lg text-black font-medium">${(item.product?.price * item.quantity).toFixed(2)}</p>
                      </div>
                    ...
                    <div className="flex justify-between items-center mt-4">
                      <div className="flex items-center border border-gray-300 h-10 w-28">
                        ...
                        >-</button>
                        <span className="w-full h-full flex items-center justify-center text-sm">{item.quantity}</span>
                        <button 
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-gray-50 p-8 rounded-sm shrink-0 sticky top-28">
              <h2 className="text-xl font-playfair font-bold text-black uppercase tracking-widest mb-6">Order Summary</h2>
