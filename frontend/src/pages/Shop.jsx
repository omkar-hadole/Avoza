...
const Shop = () => {
  ...
  useEffect(() => {
    if (products.length > 0) {
      ...
      if (searchFilter) {
        result = result.filter(p => p.name.toLowerCase().includes(searchFilter.toLowerCase()) || p.description.toLowerCase().includes(searchFilter.toLowerCase()));
      }
      ...
      <div className="flex flex-col md:flex-row gap-12">
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="sticky top-28">
            <h3 className="text-sm font-semibold tracking-widest uppercase mb-6 text-black border-b border-gray-200 pb-4">Categories</h3>
                ...
                <li key={cat}>
                  <button 
                    ...
                    className={`text-sm tracking-wide hover:text-black transition-colors ${
                      (categoryFilter === cat || (!categoryFilter && cat === 'All')) ? 'text-black font-medium' : 'text-gray-500'
                    }`}
                  >
                    {cat}
                  </button>
