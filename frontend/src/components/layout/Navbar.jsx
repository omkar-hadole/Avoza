      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="text-sm font-medium text-gray-600 hover:text-black uppercase tracking-wider transition-colors duration-200"
              >
                {link.name}
              </Link>
            ))}
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link
              to="/wishlist"
              className="relative text-gray-600 hover:text-black transition-colors duration-200 hidden sm:block"
            >
              <Heart size={20} />
              {wishlistItemCount > 0 && (
